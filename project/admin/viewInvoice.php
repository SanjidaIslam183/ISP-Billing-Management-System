...
if (isset($_POST['inputCheckedValue'])) {
    $invno=$_POST['inputCheckedValue'];
    //echo $cid;



    <?php
    $sql = "SELECT invnumber,username,startdate,enddate,item,itemqty,itemprice,totalamount FROM invoice where id=$invno";
    $result=mysqli_query($conn,$sql);
    ?>
    
    $sql="Select name,user,pass,email,cell,address from `reseller` where id=$cid";
    $result=mysqli_query($conn,$sql);
    $response=array();
    ...
    $invoiceNum="AKN_".$year."_".$cid."_".$month."_".$Mobile;
...
// Adding a row inside the tbody.
  ...
  ?>
  <option value="<?php 
  echo $rows["name"]; ?>"><?php echo $rows["name"]; ?></option>
  <?php }	?>
</select> </td>
